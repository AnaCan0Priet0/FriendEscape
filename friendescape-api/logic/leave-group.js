...
module.exports = (userId, groupId) => {
    ...
    return Promise.all([User.findById(userId), Group.findById(groupId)])
        .then(([user, group]) => {
            ...
            return Promise.all([User.findByIdAndUpdate(userId, { subbedTo: groupId }, {$set: {"subbedTo.$": 0}} ), Group.findByIdAndUpdate(groupId, { $addToSet: { subevents: userId } }).populate('subevents', 'name surname email'), User.findByIdAndUpdate(groupId, { $inc: { foults: +1 }})])
